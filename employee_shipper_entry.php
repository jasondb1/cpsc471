...
///////////////////////////////////////////////////////////////////////////////*/
	...
	$page_title		= "Shipper Data";									//***
	
	$dbtable		= $db_table_shipper;								//***
	$primaryKey		= $database->getPrimaryKey($dbtable);
	
	//*** general format of Formelements is (column in table, label, default value)
	$fShipperID   	= new FormTextField ("ShipperID", "Shipper ID", $dbtable);
	$fShipperID->setReadOnly();
	$fName		= new FormTextField ("Shname", "Shipper Name", $dbtable);
	$fName->setRequired();
	$fContact		= new FormTextField ("Ship_contact_rep", "Contact Name", $dbtable);
	$fContact->setRequired();
	$fPhone	= new FormTextField ("Shphone", "Phone", $dbtable);
	$fEmail			= new FormTextField ("Shemail", "Email", $dbtable);
	
	//Create form	
	$formObj = new FormHtml();
	
	$formObj->setTitle("Shipper Details");						//***
	$formObj->setSuccessPage('employee_shipper_view.php');		//***

	//*** set fields and groups to display in form
	$group_1 = array($fShipperID, $fName, $fContact, $fPhone, $fEmail);
	//$group_2 = array( $fEmail, $fPhone, $fStreet, $fCity, $fProvince, $fPostalCode, $fCountry);
	$groups = array('Shipper Information'=>$group_1);	//Groups are array(Name=>array(fields))
...
///////////////////////////////////////////////////////////////////////////////*/
	if (isset($_POST['submit'])){
		...
		if($values['Shname'] == "" || $values['Ship_contact_rep'] ==""){		//***
						...
						<section class="is-page-content">
							...
							<div class="row flush" style="padding:0em; padding-top:2em;">
								<div class="12u">
									...
									<div id="menu">
										<ul>
										<li><a href="employee_shipper_view.php" ><span class="button-menu"><i class="fa fa-arrow-circle-o-left fa-fw"></i>&nbsp; View Shipper Database</span></a></li>
										</ul>
									</div>
								</div>
							</div>
