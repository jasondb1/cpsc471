...
-- $Id: update-0.2.0.sql,v 1.2 2005/04/08 12:54:36 vincentarn Exp $
-- $Revision: 1.2 $
-- --------------------------------------------------------
...
-- 
-- RELEASE NOTE
-- This script should be run ONLY if you're upgrading from version 0.1.3 to 0.2.0.
...
ALTER TABLE `phpauthent_groups` ADD `description` VARCHAR( 80 );

ALTER TABLE `phpauthent_users` ADD `realname` VARCHAR( 80 ) ,
ADD  `email` varchar(120) default NULL,
ADD  `lastlogin` DATETIME,
ADD  `creation` DATE,
ADD  `numlogins` INT DEFAULT '0' NOT NULL;