...
///////////////////////////////////////////////////////////////////////////////*/	
	//get and set initial variables
		...
		$dbtable		= $db_table_components;						//*** In _config.php
...
/////////////////////////////////////////////////*/
	...
	$columns = array(
		...
		array('columnName'=>'reorder_qty',			'displayName'=>'Reorder Qty', 				'type'=>'text')
	);
	...
	if ($search != ""){
		...
		foreach ($columns as $row){
			if ($i == 0){ 
				$where = " Components." . $row['columnName'] . " LIKE '%$search%'";
			}
			else {
				$where .= " OR Components." . $row['columnName'] . " LIKE '%$search%'";
			}
			$i++;
		}	
	}
	...
	if ($orderBy == ''){ $orderBy = "Components.part_no";}
	
	$sql = "SELECT * FROM Components JOIN Inventory ON Components.part_no = Inventory.part_no WHERE " . $where .	" ORDER BY " . $orderBy . " " .$direction;
	...
	$table->editPage="employee_components_entry.php";		//***
				...
				<section class="is-page-content">
					<div class="row flush" style="padding:0em; padding-top:2em;">
						<div class="12u">
							...
							<div id="menu">
								<ul>
								<li><a href="employee_components_entry.php" ><span class="button-menu"><i class="fa fa-plus fa-fw"></i>&nbsp; Add Component</span></a></li>
								</ul>
							</div>
							<hr>
						</div>
