...
///////////////////////////////////////////////////////////////////////////////*/
	...
	$page_title		= "Vendor Data";							//***
	
	$dbtable		= $db_table_vendor;								//***
	$primaryKey		= $database->getPrimaryKey($dbtable);
	
	//*** general format of Formelements is (column in table, label, default value)
	$fVendorID   	= new FormTextField ("Vendor_ID", "Vendor ID", $dbtable);
	$fVendorID->setReadOnly();
	$fName			= new FormTextField ("Name", "Name", $dbtable);
	...
	$fContact		= new FormTextField ("Contact", "Contact", $dbtable);
	...
	$formObj->setTitle("Vendor Details");						//***
	$formObj->setSuccessPage('employee_vendor_view.php');		//***

	//*** set fields and groups to display in form
	$group_1 = array($fVendorID, $fName, $fContact, $fEmail, $fPhone);
	$group_2 = array( $fStreet, $fCity, $fProvince, $fPostalCode);
	$group_3 = array( $fNotes);
	$groups = array('Information'=>$group_1, 'Vendor Details'=>$group_2, 'Notes'=>$group_3);	//Groups are array(Name=>array(fields))
						...
						<section class="is-page-content">
							...
							<div class="row flush" style="padding:0em; padding-top:2em;">
								<div class="12u">
									...
									<div id="menu">
										<ul>
										<li><a href="employee_vendor_view.php" ><span class="button-menu"><i class="fa fa-arrow-circle-o-left fa-fw"></i>&nbsp; View Vendors</span></a></li>
										</ul>
									</div>
								</div>
							</div>
