		...
		foreach ($company_databases as $key=>$value){	
			...
			while($row = $result->fetch_assoc()){
			  ...
			  $require_subdivision = $row['require_subdiv'];
				if ($employee !=""){
					$table .= "<a href=\"employee_time_entry.php?edit_record=$jobnumber&description=$description&customer=$customer&require_division=$require_division&require_subdivision=$require_subdivision&employee=$employee&date=$date&time_in=$time_in&time_out=$time_out\" ><b>$jobnumber</b>: $location - $description</a><br><hr>";
				}
				else{
					$table .= "<a href=\"employee_time_entry.php?edit_record=$jobnumber&description=$description&customer=$customer&require_division=$require_division&require_subdivision=$require_subdivision&time_in=$time_in&time_out=$time_out\" ><b>$jobnumber</b>: $location - $description</a><br><hr>";
				}
			}	//end while
			$table .= "<br><hr>";
		}
					...
					<div class="row">
						...
						<section class="is-page-content">
							...
							<div class="row flush" style="padding:0em;">					
								<div class="12u">
									<hr>
									<span class="hangingHead">Timelog</span><br>
									<!-- Table Controls/Options -->
									<form style="background-color:#FFFFCC;" method="get" action="<?php $_SERVER['PHP_SELF'] ?>" name="options">
									<?php 
										echo '<input border-style:solid; border-width:medium;" type="text" name="search" placeholder="Search" value="'.$search. '">';
