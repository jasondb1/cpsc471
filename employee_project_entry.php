...
///////////////////////////////////////////////////////////////////////////////*/
	...
	$dbtable		= $db_table_project;								//***
	$primaryKey		= $database->getPrimaryKey($dbtable);
	
	//*** general format of Formelements is (column in table, label, default value)
	$fProjectNo   	= new FormTextField ("ProjectNo", "Project Number", $dbtable);
	$fProjectNo->setReadOnly();
	$fDescription	= new FormTextField ("Description", "Description", $dbtable);
	$fStatus		= new FormSelect ("Status", "Status", $project_status, $dbtable);
	$fStartDate		= new FormDateField ("StartDate", "Start Date", $dbtable);
	$fEndDate		= new FormDateField ("EndDate", "Comp. Date", $dbtable);
	$fNotes			= new FormtextBox ("notes", "Notes", $dbtable);
	$e_list = getEmployeeNames();
	$fSupervisorID	= new FormSelect ("SupervisorID", "Supervisor", $e_list, $dbtable, "In Progress");
	$fFilePath		= new FormTextField ("filepath", "Filepath", $dbtable);

	
	//Create form	
	$formObj = new FormHtml();
	
	$formObj->setTitle("Project Details");						//***
	$formObj->setSuccessPage('employee_project_view.php');		//***

	//*** set fields and groups to display in form
	$group_1 = array($fProjectNo, $fDescription, $fStatus);
	$group_2 = array($fStartDate, $fEndDate, $fSupervisorID, $fFilePath);
	$group_3 = array($fNotes);
	$groups  = array('Project Information'=>$group_1, 'Project Details'=>$group_2, 'Notes'=>$group_3);	//Groups are array(Name=>array(fields))
	...
	<head>
		...
		<style type="text/css">
		  #datepicker1, #timepicker1, #timepicker2{
		   ...
		   #datepicker2 {background-image:url("images/calendar.png");}
		</style>
						...
						<section class="is-page-content">
							...
							<div class="row flush" style="padding:0em; padding-top:2em;">
								<div class="12u">
									...
									<div id="menu">
										<ul>
									<li><a href="employee_project_view.php" ><span class="button-menu"><i class="fa fa-arrow-circle-o-left fa-fw"></i>&nbsp; View Project Database</span></a></li>
										</ul>
									</div>
								</div>
							</div>
